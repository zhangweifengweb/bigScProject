...
export default {
  ...
  created() {
    ...
    this.init();
    this.socketApi.initWebSocket("抢修指挥", this.getConfigResult);
    this.$store.state.navTitle='固原供电公司抢修指挥';
  },
  destroyed(){
    clearInterval(this.timer)
  },
  methods: {
    ...
    getQXZH(){
      this.$store.dispatch("init", { url: "/qxzh" }).then((res) => {
        if (res.code == 200) {
          this.initData = res.data
        }
      })
      // axios.get('/JBTSHJ/GuYuanScreen/getScreenById/qxzh').then(res=>{
      //   // console.log(res)
...
.main_content {
  height: 1080px;
  width: 5760px;
  background:url('/image/emergencyRepair/bj.png') no-repeat;
  .top{
    color: #B1CAEA;
    font-size: 23px;
    top: 49px;
